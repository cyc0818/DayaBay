...
void nCFitter(){
    ...
    minimizer->DefineParameter(0, "CS_nC_AD", 0.00500734, 0.0002, 0.004, 0.008);
    minimizer->DefineParameter(1, "pull_acc_1", 0, 0.01, -10000, 10000);
    ...
    minimizer->DefineParameter(29, "pull_eff_C_corr", 0, 0.01, -10, 10);
    minimizer->DefineParameter(30, "pull_eff_C_uncorr_1", 0, 0.01, -10, 10);
    minimizer->DefineParameter(31, "pull_eff_C_uncorr_2", 0, 0.01, -10, 10);
    minimizer->DefineParameter(32, "pull_eff_C_uncorr_3", 0, 0.01, -10, 10);
    minimizer->DefineParameter(33, "pull_eff_C_uncorr_4", 0, 0.01, -10, 10);
...
void fitter(int& npar, double* gin, double& f, double* par, int iflag){
    ...
    const double pull_eff_C_corr = par[29];
    const array<double,5> pull_eff_C_uncorr = {0, par[30], par[31], par[32], par[33]};
    ...
    for(int ad = 1; ad<5; ad++){
        ...
        //cout << "w_LS = " << w_LS << endl;
        //cout << "w_acry = " << w_Acrylic << endl;


        const double ExpnC = nC_eff_uc.GetValue()*(1.+pull_eff_C_corr+pull_eff_C_uncorr.at(ad))*CS_nC_AD
                             *(w_GdLS*GdLS.at(ad).GetC()*(1.+pull_GdLS_C.at(ad))
                                +w_LS*LS.at(ad).GetC()*(1.+pull_LS_C.at(ad))+w_Acrylic*Acrylic.at(ad).GetC());
        const double ExpnGd = nGd_eff_uc.GetValue()*(1.+pull_eff_Gd_corr+pull_eff_Gd_uncorr.at(ad))
                            * w_GdLS*(GdLS.at(ad).GetGd(155)*(1.+pull_GdLS_Gd.at(ad))*nGd155_CS_th.at(0).GetValue()*(1.+pull_cs_Gd155)
                            + GdLS.at(ad).GetGd(157)*(1.+pull_GdLS_Gd.at(ad))*nGd157_CS_th.at(0).GetValue()*(1.+pull_cs_Gd157));
        const double r_CGd = ExpnC/ExpnGd;
        //cout << "exp nGd = " << ExpnGd << endl;


        const Double_e errcal = nC_can_sta.at(ad)-nC_acc_sta.at(ad)-nC_lkg_sta.at(ad)+nGd_can_sta.at(ad)-nGd_acc_sta.at(ad);
        const double stat_err = errcal.GetError();
        const double MeasnC = nC_can_sta.at(ad).GetValue()-nC_acc_sta.at(ad).GetValue()*(1.+pull_acc.at(ad))
                              -nC_lkg_sta.at(ad).GetValue()*(1.+pull_lkg.at(ad));
        const double MeasnGd = nGd_can_sta.at(ad).GetValue()-nGd_acc_sta.at(ad).GetValue()*(1.+pull_acc.at(ad));
        //cout << "measured ratio = " << MeasnC/MeasnGd << endl;


        const double delta = (MeasnC-MeasnGd*r_CGd)/stat_err;
        f += (pow(delta,2) + pow(pull_acc.at(ad)/sys_acc.at(ad),2) + pow(pull_lkg.at(ad)/sys_lkg_C.at(ad),2)
             ...
             + pow(pull_LS_C.at(ad)/sys_LS_C.at(ad),2) + pow(pull_eff_C_uncorr.at(ad)/sys_eff_C_uncorr,2)
    ...
    f += pow(pull_eff_C_corr/sys_eff_C_corr,2);
