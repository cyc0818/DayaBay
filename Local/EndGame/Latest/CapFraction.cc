...
void CapFraction(){
    ...
    map<int,pair<int,string>> ZAmap = {{1,{1,"H"}}, {12,{6,"C"}}, {155,{64,"Gd"}}, {157,{64,"Gd"}}};
    ...
    for(auto s : ZAmap){
        ...
        int i = 0;
        int points = 0;
        ifstream infile(TString::Format("/Users/chengyuchin/Downloads/%s%d-102.dat",s.second.second.c_str(),s.first));
        while(infile.good()){
            i++;
            getline(infile, line);
            if(i < 6) continue;
            ...
            for(int j = 0; j<7; j++){
                ...
                if(p.first and p.second){
                    ...
                    if(s.first == 157) En.push_back(p.first);
                    p = {0,0};
                    points++;
                }
