void MCMerge(){
    ...
    string DirPath = "/dybfs2/users/yuchincheng/workdir/IBD/data/MC3142/noMOcap_sq/";
    string bashPath = "/dybfs2/users/yuchincheng/workdir/IBD/code/Merge/MCbashFile/";
    string InputFile = DirPath + "FileList.txt";
    string filename;
    int a = 0;
    int b = 0;
    TString OutputFile = static_cast<TString>(bashPath) + TString::Format("merge_%d.sh",b);
    string MergeFinalPath = DirPath + "Merge/";


    system(("ls -I FileList.txt -I Merge -1 " + DirPath + " >> " + InputFile).c_str());
    ...
    Fout << "hadd -ff " << MergeFinalPath << "MergePlots_" << b << ".root ";
