...
const double  isolation = 0.0008; // s
const double  capTime   = 0.0004; // s
const double  be4DelayCut= 2.0*capTime;
const double  aftDelayCut= 1.0*capTime;
/*const double  capTime   = 0.001; // s
const double  be4DelayCut= 0.0014;
const double  aftDelayCut= 0.0004;*/
const double  Ep1 = 0.7;
const double  Ep2 = 12;
const double  Ed1 = 1.5;
const double  Ed2 = 12;
int main(int argc,char** argv) {
    ...
    double  LiveTime = 0;
    double  FullTime = 0;
    bool forbidden[5] = {true,false,false,false,false};
    ...
    //m_ibdTree->Branch( "CellNop",    &CellNop,     "CellNop/I" );
    ...
    //m_ibdTree->Branch( "CellNod",    &CellNod,     "CellNod/I" );
    ...
    //m_sigTree->Branch( "CellNos",    &CellNos,     "CellNos/I" );
    ...
    for( long long jentry = 0; jentry < nevent; jentry++ ) {
        aEvent->GetEntry(jentry);
        
        //if (E<0.7 || E>12.0) continue;
        
        if(PrevEvt.find(Det) == PrevEvt.end()) PrevEvt.insert(pair<int,vector<EventBrief>>(Det,vector<EventBrief>()));
        int nEvt = PrevEvt[ Det ].size();
        //cout<<"nEvt: "<<nEvt<<endl;
        if (nEvt==4 and !forbidden[Det]) {
            ...
            if (Eprompt>Ep1 && Eprompt<Ep2 &&
            	...
            	/*if ( preEs>Ep1 && preEs<Ep2 ) ibdPLikeS  = true;        // similar to nGd isolation
            	...
            	if ( preEs>Ep1 && preEs<Ep2 ) ibdPLikeS  = true;      // similar to nH isolation
            	if ( aftEs>Ep1 && aftEs<Ep2 ) ibdDLikeS = true;
                if ( (tB4Es < isolation && ibdPLikeS )||
                     (tAfEs < isolation && ibdDLikeS ) )    SinglPassDMC = false;


                if ( Esingles>Ep1 && Esingles<Ep2 && SinglPassDMC) {
                    ...
                    //CellNos = GetCellNo(PrevEvt[ Det ][1]);
        ...
        if(static_cast<int>(E) == -1 and TrigNano == -1){
            map<int,vector<EventBrief>>::iterator it = PrevEvt.find(Det);
            PrevEvt.erase(it);
            forbidden[Det] = true;
            continue;
        }
    ...
    ///
    m_ibdTree->Write(0,TObject::kOverwrite);
    m_sigTree->Write(0,TObject::kOverwrite);
    ofile->Close();
    delete ofile;
