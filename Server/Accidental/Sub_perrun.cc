...
const int correction = 3;   // 0 for not correction, 1 for Poisson+lambert_w0 correction (DocDB 12566 pg.92), 2 for Poisson correction
...
TMultiGraph *mg8 = new TMultiGraph("mg8","isoEff");
TMultiGraph *mg9 = new TMultiGraph("mg9","ucRate");
...
double nHAcc(double sRate, double mRate, double Tc){
    ...
    double term3 = -1.*fracTerm3*exp( - mRate*Tc )*(1-ExpTerm2);
...
void fitter(int& npar, double* gin, double& f, double* par, int iflag){
    ...
    const double Riso = par[0];
    const double Riso_err = par[1];
    const double Rs = par[2];
    const double Rmu = par[3];
    const double Tc = par[4];


    f = (Riso-Rs*nHmEff(Rs,Rmu,Tc)) * (Riso-Rs*nHmEff(Rs,Rmu,Tc)) /Riso_err/Riso_err;

}


Double_e GetTrueRs(Double_e R1fold, Double_e Rmu, double Tc){

    double result = 0;
    double result_err = 0;


    unique_ptr<TMinuit> mini(new TMinuit(5));
    mini->SetFCN(fitter);
    mini->DefineParameter(0, "R1fold", R1fold.GetValue(), 0, 0, 2*R1fold.GetValue());
    mini->DefineParameter(1, "R1fold_err", R1fold.GetError(), 0, 0, 2*R1fold.GetError());
    mini->DefineParameter(2, "Rs", 1.1*R1fold.GetValue(), R1fold.GetError(), R1fold.GetValue(), 2*R1fold.GetValue());
    mini->DefineParameter(3, "Rmu", Rmu.GetValue(), Rmu.GetError(), 0, 2*Rmu.GetValue());
    mini->DefineParameter(4, "Tc", Tc, 0, 0, 2*Tc);
    ...
    mini->Command("MIGRAD");
    mini->Command("MINOS");
    mini->GetParameter(2,result,result_err);


    return Double_e(result,result_err);
    ...
    for (int nSite=1; nSite<4; nSite++) {
        ...
        while (!infile.eof()) {
            ...
            while(infile.getline(line,sizeof(line),'\n')){
                ...
                double NetMu = eachValue[10]*1.0;   // check the txt file, may have some problems in this column (have checked on July 12 2022)
                ...
                if (key!=0 && RunNo<90000 ) //80000 for p19a
                {
                    ...
                    if(correction == 0) Eff_iso = 1;
                        ...
                        if(symmetric_iso){
                            ...
                            Eff_iso = exp(boost::math::lambert_w0(-2.*(s07*isolation)/LiveT));
                        ...
                        if(symmetric_iso){
                            const Double_e temp(GetTrueRs(s07e/LiveT/gapEff,Rmue,isolation));
                            result = s07e/LiveT/gapEff/temp;
                        }
                        else{
                            cerr << "Error! Method 3 can only used in symmatric case" << endl;
                            return 0;
                        }
                        ...
                        cerr << "Error! Correction method should be 0 or 1 or 2 or 3" << endl;
                        return 0;
                    }
                    // BCW Bei-Zhen OG:
                    ...
                    // uncertainty
                    const Double_e Rs07e = s07e/LiveT/gapEff;
                    ...
                    //const Double_e Rs07e = s07e/LiveT;
                    ...
                    const double SF    = Nacc/(1.0*totalAccSample);
                    ...
                    const Double_e SFe = Nacce/Double_e(1.*totalAccSample, sqrt(1.*totalAccSample));
                    ...
                    const Double_e accdr_cut = Double_e(1.*hAccDist->Integral(1,50),sqrt(1.*hAccDist->Integral(1,50)))
                                               /Double_e(1.*hAccDist->GetEntries(),sqrt(1.*hAccDist->GetEntries())); 
                    const Double_e NaccCute = Nacce*accdr_cut;
                    const Double_e RaccCute = Racce*accdr_cut;
                    ...
                    double RnIbd = Nibd*1.0/(1.0*LiveT*gapEff);
                    double RnIbd_err = sqrt( Nibd )*1.0/(1.0*LiveT*gapEff);
                    const Double_e RnIbde(RnIbd,RnIbd_err);
                    const Double_e RnIbd2e = RnIbde-RaccCute;
                    ...
                    Nsg_rate[gAdNo]->SetPoint(nPoints[gAdNo],  Date, Rs07e.GetValue());
                    Nsg_rate[gAdNo]->SetPointError(nPoints[gAdNo],0, Rs07e.GetError());

                    iso_eff[gAdNo]->SetPoint(nPoints[gAdNo],  Date, Eff_iso);
                    iso_eff[gAdNo]->SetPointError(nPoints[gAdNo],0, Eff_iso_err);

                    Nuc_rate[gAdNo]->SetPoint(nPoints[gAdNo],  Date, R07e.GetValue());
                    Nuc_rate[gAdNo]->SetPointError(nPoints[gAdNo],0, R07e.GetError());
                    
                    multi_eff[gAdNo]->SetPoint(nPoints[gAdNo],  Date, multiEffe.GetValue());
                    multi_eff[gAdNo]->SetPointError(nPoints[gAdNo],0, multiEffe.GetError());
                    ...
                    Nibd_rate[gAdNo]->SetPoint(nPoints[gAdNo],  Date, RnIbde.GetValue()*86400.);
                    Nibd_rate[gAdNo]->SetPointError(nPoints[gAdNo],0, RnIbde.GetError()*86400.);
                    
                    ibdRate2[gAdNo]->SetPoint(nPoints[gAdNo],  Date, RnIbd2e.GetValue()*86400.);
                    ibdRate2[gAdNo]->SetPointError(nPoints[gAdNo],0, RnIbd2e.GetError()*86400.);
...
int EndJob()
{
    ...
    for (int nAd=1; nAd<9; nAd++) {
        ...
        Nmu_rate[nAd]->SetMarkerStyle(8);
        Nmu_rate[nAd]->SetTitle(TString::Format("EH%dAD%d",EH,AD));
        
        Nsg_rate[nAd]->SetMarkerSize(1);
        Nsg_rate[nAd]->SetMarkerColor(nAd);
        Nsg_rate[nAd]->SetLineColor(nAd);
        Nsg_rate[nAd]->SetMarkerStyle(8);
        Nsg_rate[nAd]->SetTitle(TString::Format("EH%dAD%d",EH,AD));

        Nuc_rate[nAd]->SetMarkerSize(1);
        Nuc_rate[nAd]->SetMarkerColor(nAd);
        Nuc_rate[nAd]->SetLineColor(nAd);
        Nuc_rate[nAd]->SetMarkerStyle(8);
        Nuc_rate[nAd]->SetTitle(TString::Format("EH%dAD%d",EH,AD));
        ...
        Nibd_rate[nAd]->SetMarkerStyle(8);
        Nibd_rate[nAd]->SetTitle(TString::Format("EH%dAD%d",EH,AD));
        
        veto_eff[nAd]->SetMarkerSize(1);
        veto_eff[nAd]->SetMarkerColor(nAd);
        veto_eff[nAd]->SetLineColor(nAd);
        veto_eff[nAd]->SetMarkerStyle(8);
        veto_eff[nAd]->SetTitle(TString::Format("EH%dAD%d",EH,AD));

        iso_eff[nAd]->SetMarkerSize(1);
        iso_eff[nAd]->SetMarkerColor(nAd);
        iso_eff[nAd]->SetLineColor(nAd);
        iso_eff[nAd]->SetMarkerStyle(8);
        iso_eff[nAd]->SetTitle(TString::Format("EH%dAD%d",EH,AD));
        
        multi_eff[nAd]->SetMarkerSize(1);
        multi_eff[nAd]->SetMarkerColor(nAd);
        multi_eff[nAd]->SetLineColor(nAd);
        multi_eff[nAd]->SetMarkerStyle(8);
        multi_eff[nAd]->SetTitle(TString::Format("EH%dAD%d",EH,AD));
        
        ibdRate2[nAd]->SetMarkerSize(1);
        ibdRate2[nAd]->SetMarkerColor(nAd);
        ibdRate2[nAd]->SetLineColor(nAd);
        ibdRate2[nAd]->SetMarkerStyle(8);
        ibdRate2[nAd]->SetTitle(TString::Format("EH%dAD%d",EH,AD));
    ...
    mg1->Draw("AP");
    mg1->GetXaxis()->SetLimits(1320*1e6,1515*1e6);
    mg1->GetXaxis()->SetTimeDisplay(1);
    mg1->GetXaxis()->SetTimeFormat("%b-20%y%F1970-01-01 00:00:00");
    mg1->GetXaxis()->SetNdivisions(712);
    mg1->GetYaxis()->SetTitle("R_{#mu}(s^{-1})");
    mg1->GetXaxis()->SetTitle("Date");
    mg1->GetXaxis()->CenterTitle(kTRUE);
    mg1->GetYaxis()->CenterTitle(kTRUE);
    if(SavePlots && Postsub) c1->SaveAs( outputpath+"MuRate.png");
    
    TCanvas *c2 = new TCanvas(TString::Format("c2"),TString::Format("c2"),800,600);
    mg2->Draw("AP");
    mg2->GetXaxis()->SetLimits(1320*1e6,1515*1e6);
    mg2->GetXaxis()->SetTimeDisplay(1);
    mg2->GetXaxis()->SetTimeFormat("%b-20%y%F1970-01-01 00:00:00");
    mg2->GetXaxis()->SetNdivisions(712);
    mg2->GetYaxis()->SetTitle("R_{s,0.7}(s^{-1})");
    ...
    mg3->Draw("AP");
    mg3->GetXaxis()->SetLimits(1320*1e6,1515*1e6);
    mg3->GetXaxis()->SetTimeDisplay(1);
    //mg3->GetXaxis()->SetTimeFormat("%b-%d");
    mg3->GetXaxis()->SetTimeFormat("%b-20%y%F1970-01-01 00:00:00");
    mg3->GetXaxis()->SetNdivisions(712);
    mg3->GetYaxis()->SetTitle("#epsilon_{#mu}");
    ...
    mg4->Draw("AP");
    mg4->GetXaxis()->SetLimits(1320*1e6,1515*1e6);
    mg4->GetXaxis()->SetTimeDisplay(1);
    mg4->GetXaxis()->SetTimeFormat("%b-20%y%F1970-01-01 00:00:00");
    mg4->GetXaxis()->SetNdivisions(712);
    mg4->GetYaxis()->SetTitle("#epsilon_{m}");
    ...
    mg6->Draw("AP");
    mg6->GetXaxis()->SetLimits(1320*1e6,1515*1e6);
    mg6->GetXaxis()->SetTimeDisplay(1);
    mg6->GetXaxis()->SetTimeFormat("%b-20%y%F1970-01-01 00:00:00");
    mg6->GetXaxis()->SetNdivisions(712);
    mg6->GetYaxis()->SetTitle("R_{Can}(day^{-1})");
    ...
    mg7->Draw("AP");
    mg7->GetXaxis()->SetLimits(1320*1e6,1515*1e6);
    mg7->GetXaxis()->SetTimeDisplay(1);
    mg7->GetXaxis()->SetTimeFormat("%b-20%y%F1970-01-01 00:00:00");
    mg7->GetXaxis()->SetNdivisions(712);
    mg7->GetYaxis()->SetTitle("R_{IBD}(day^{-1})");
    mg7->GetXaxis()->SetTitle("Date");
    mg7->GetYaxis()->SetLimits(-500,5000);
    mg7->GetXaxis()->CenterTitle(kTRUE);
    mg7->GetYaxis()->CenterTitle(kTRUE);
    ...
    mg8->GetYaxis()->SetTitle("#epsilon_{i}");
    mg8->GetXaxis()->SetTitle("Date");
    mg8->GetXaxis()->CenterTitle(kTRUE);
    mg8->GetYaxis()->CenterTitle(kTRUE);
    
    TCanvas *c9 = new TCanvas(TString::Format("c9"),TString::Format("c9"),800,600);
    mg9->Draw("AP");
    mg9->GetXaxis()->SetLimits(1320*1e6,1515*1e6);
    mg9->GetXaxis()->SetTimeDisplay(1);
    mg9->GetXaxis()->SetTimeFormat("%b-20%y%F1970-01-01 00:00:00");
    mg9->GetXaxis()->SetNdivisions(712);
    mg9->GetYaxis()->SetTitle("R_{uc,0.7}(s^{-1})");
    mg9->GetXaxis()->SetTitle("Date");
    mg9->GetYaxis()->SetLimits(-500,5000);
    mg9->GetXaxis()->CenterTitle(kTRUE);
    mg9->GetYaxis()->CenterTitle(kTRUE);
    ...
    TFile *ttfile = new TFile(TString::Format("%s_Plots_Poisson.root",dataPeriod.c_str()),"recreate");
    for (int nAd=1; nAd<9; nAd++) {
