...
double  capTime   = 0.00045; // s --->for uncertainty studies
...
double  Epl	= 0.7;
double  Eph	= 12.0;
double  Edl	= 1.5;//*(1-0.002);
double  Edh	= 12.0;//*(1+0.002);
...
int main(int argc,char** argv) {
    ...
    for( int AdNo=1; AdNo<=4; AdNo++ )  {
        ...
        for( int idx=0; idx<nEvt/2-1; idx++ )  {
            ...
            if ( Ed_acc>Edl && Ed_acc<Edh )  h1dDpd_acc[AdNo]->Fill(dist_acc);
            //inverse:
            if ( Ep_acc>Edl && Ep_acc<Edh )  h1dDpd_acc[AdNo]->Fill(dist_acc);
               
            if ( dist_acc>distCut) continue;
            if ( Ed_acc>Edl && Ed_acc<Edh )  h2dEpEd_acc[AdNo]->Fill( Ed_acc, Ep_acc );
            if ( Ep_acc>Edl && Ep_acc<Edh )  h2dEpEd_acc[AdNo]->Fill( Ep_acc, Ed_acc );
            //////////////
